...
class CartController extends GetxController {
  ...
  void addToCart(ProductModel product) {
    if (productQuantityInCart.value < 1) {
      MLoaders.customToast(message: 'Selecione a Quantidade');
      return;
    }

    if (product.productType == ProductType.variable.toString() &&
        variationController.selectedVariation.value.id.isEmpty) {
      MLoaders.customToast(message: 'Selecione uma Variação');
      return;
    }

    if (product.productType == ProductType.variable.toString()) {
      if (variationController.selectedVariation.value.stock < 1) {
        MLoaders.warningSnackBar(message: 'Fora de estoque', title: 'Ah não!');
        return;
      }
    } else {
      if (product.stock < 1) {
        MLoaders.warningSnackBar(title: 'Ah não!', message: 'Fora de estoque');
        return;
      }
    }
    ...
    updateCart();
    MLoaders.customToast(message: 'Adicionado ao carrinho!');
  }
  ...
  void removeFromCartDialog(int index) {
    Get.defaultDialog(
      ...
      onConfirm: () {
        cartItems.removeAt(index);
        updateCart();
        MLoaders.customToast(message: 'Produto removido do carrinho!');
        Get.back();
      },
      onCancel: () => () => Get.back(),
    );
